...
def train(X, Y, rates):
    ...
    for learning_rate in rates:
        ...
        print('Evaluating learning rate %g' % learning_rate)
        print('===========================')
        # Create the model
    ...
    optimal = rates[np.argmin(mean_err)]
    print()
...
def main():
    ...
    hidden_counts = np.zeros(len(rates))
    ...
    final_optimal = rates[np.argmax(hidden_counts)]
    print('After %d experiments the optimal learning rate is %g' % (no_exps, final_optimal))

    plt.figure(1)
    plt.semilogy(range(no_exps), optimal_neurons, marker='x', linestyle='None')
    plt.xticks(range(no_exps), np.arange(no_exps) + 1)
    plt.xlabel('experiment')
    plt.ylabel('optimum learning rates')
    plt.savefig('./figures/PARTB_QNS2_a.png')
    ...
    plt.savefig('./figures/PARTB_QNS2_b.png')

    i = 3
    for cv_error in cv_errors:

        plt.figure(i)
        plt.plot(range(len(rates)), cv_error, 'x')
        plt.xticks(range(len(rates)), ['0.5e-6', '1e-7', '0.5e-8', '1e-9', '1e-10'])
        plt.xlabel('Learning Rates')
        plt.ylabel('Mean CV errors')
        plt.title('Mean CV errors VS Learning Rates EXP %d' % (i-2))
        plt.savefig('./figures/PARTB_QNS2_c_EXP_%d.png' % (i-2))
        i += 1
...
def test_optimal(X,Y,pureTestX,pureTestY,optimal):
    ...
    optimizer = tf.train.GradientDescentOptimizer(optimal)
    train_op = optimizer.minimize(loss)
