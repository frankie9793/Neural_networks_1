...
def train(X, Y, puretestX, puretestY,prob):
    ...
    w1 = tf.Variable(tf.truncated_normal([num_features, optimal_hidden],
                                         stddev=1.0 / math.sqrt(float(num_features))),
                     name='weights')

    b1 = tf.Variable(tf.zeros([optimal_hidden]), name='biases')
    ...
    w3 = tf.Variable(tf.truncated_normal([hidden_units, hidden_units],
                                         stddev=1.0 / math.sqrt(float(hidden_units))),
                     name='weights')

    b3 = tf.Variable(tf.zeros([hidden_units]), name='biases')

    h3 = tf.nn.relu(tf.matmul(h2_dropout, w3) + b3)
    h3_dropout = tf.nn.dropout(h3, keep_prob)

    w4 = tf.Variable(tf.truncated_normal([hidden_units, 1],
    ...
    for i in range(5):
        ...
        with tf.Session() as sess:
            ...
            for i in range(epochs):
                ...
                if i % 100 == 0:
                    print('iter %d: test error without dropping %g' % (i, test_err_nodrop[i]))
...
def main():
    ...
    plt.savefig('./figures/MTE_5LAYER.png')
